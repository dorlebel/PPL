...
it('evaluates recursive procedures', () => {
    ...
    */
    ...
    it('evaluates programs which would give an error in applicative order, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (define f (lambda (x) (-)))
                1)`), evalNormalProgram)).to.deep.equal(makeOk(1));
    });
    
    it('evaluates programs which would goes into an inifinite loop, but not in normal order', () => {
        expect(bind(parseL4(`
            (L4 (define loop (lambda (x) (loop x)))
                (define f (lambda (x) 8))
                (f (loop 0)))`), evalNormalProgram)).to.deep.equal(makeOk(8));
    });

    it('in normal order the evaluation is less efficient than in applicative', () => {
        expect(bind(parseL4(`
            (L4 ((lambda (x) (+ x x))(* 2 3)))`), evalNormalProgram)).to.deep.equal(makeOk(12));
    });
