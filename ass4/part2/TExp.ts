...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? (texps.length === 1) && (texps[0] === 'Empty') ? makeOk(makeEmptyTupleTExp()) : 
                                bind(parseTupleTExp(texps), (t : TExp[])=> allT(isNonTupleTExp, t) ? makeOk(makeNonEmptyTupleTExp(t)) : makeFailure("invalid tuple") ) :
