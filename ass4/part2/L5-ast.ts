...
//         |  ( values <cexp>+)               / valExp(vals: CExp[])
//         |  ( let-value (<vBinding>*) <cexp>+ )          / valExp(vals: CExp[])
// <binding>  ::= ( <var> <cexp> )            / Binding(var:VarDecl, val:Cexp)
// <vBinding>  ::= ( <var>* <cexp> )            / Binding(vars:VarDecl[], val:Cexp)
...
export interface VBinding {tag: "VBinding"; var: VarDecl[]; val: CExp; }
export const makeVBinding = (v: VarDecl[], val: CExp): VBinding =>
    ({tag: "VBinding", var: v, val: val});
export const isVBinding = (x: any): x is VBinding => x.tag === "VBinding";
...
export interface LetValuesExp {tag: "LetValuesExp"; bindings: VBinding[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: VBinding[], body: CExp[]): LetValuesExp =>
...
export const makeValuesExp = (v: CExp[]): ValuesExp =>
    ({tag: "ValuesExp", vals: v});
export const isValuesExp = (x: any): x is ValuesExp => x.tag === "ValuesExp";

// export interface LetValuesExp {tag: "LetValuesExp"; bindings: Binding[]; body: CExp[]; }
...
const parseValuesExp = (params: Sexp[]): Result<ValuesExp> => 
    bind(mapResult(parseL5CExp, params), (v: CExp[]) => makeOk(makeValuesExp(v)))
...
! isGoodBindings(bindings) ? makeFailure(`Invalid Vbindings: ${JSON.stringify(bindings)}`) :
safe2((bdgs: VBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
    (parseVBindings(bindings), mapResult(parseL5CExp, body));
...
const parseVBindings = (vbindings: [Sexp, Sexp][]): Result<VBinding[]> =>//TODO?
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeVBinding, vds, vals)))
        ( mapResult( (vb : Sexp) => isArray(vb)?  mapResult(parseVarDecl, vb) : mapResult(parseVarDecl, [vb]), map(b => b[0], vbindings)) , mapResult(parseL5CExp, map(b => b[1], vbindings)));
...
const unparseVBindings = (bindings: VBinding[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string[], val: string) => makeOk(`((${join(" ",vd)}) ${val})`))(mapResult(unparseVarDecl,bdg.var), unparse(bdg.val)), bindings),
         (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseValuesExp = (vl: ValuesExp): Result<string> =>
        bind (mapResult(unparse, vl.vals), (s: string[]) => makeOk(`(values ${join(" ", s)})`));
...
const unparseLetValuesExp = (le: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseVBindings(le.bindings), unparseLExps(le.body));
