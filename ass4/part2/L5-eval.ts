...
import { map, repeat, zipWith, concat, reduce } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, ValuesExp, isLetValuesExp, LetValuesExp } from './L5-ast';
...
const evalValues = (exp: ValuesExp, env: Env): Result<Tuple> =>
    bind( mapResult( (v) => applicativeEval(v, env), exp.vals), (v : SExpValue[]) => makeOk(makeTuple(v)));
...
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    ...
    const evaluatedValues: Result<Value[]> = (mapResult((val) => applicativeEval(val, env), values));
    const valuesAsSexp: SExpValue[] = isOk(evaluatedValues)?
    allT(isTuple, evaluatedValues.value)?
        reduce(
            (acc: SExpValue[], currTuple: Tuple) => reduce(
                    (secondAcc: SExpValue[], valueInTuple:SExpValue) => secondAcc.concat(valueInTuple),
                     acc, 
                     currTuple.vals), 
            [], 
    ...
    if (valuesAsSexp.length !== variables.length)
        return makeFailure(`The number of values and the number of varibles dont match`);
    return evalSequence(exp.body, makeExtEnv(map((v: VarDecl) => v.var, variables) ,valuesAsSexp ,env));
};
