...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, isValuesExp, ValuesExp, LetValuesExp, CExp, VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeEmptyTupleTExp, isNonTupleTExp, makeNonEmptyTupleTExp, isNonEmptyTupleTExp } from "./TExp";
...
import { Value, SExpValue } from './L5-value';
...
export const typeofValues = (valExp: ValuesExp, tenv: TEnv): Result<TExp> => 
    isEmpty(valExp.vals)? makeOk(makeEmptyTupleTExp()) :
        bind( mapResult( (vals) => typeofExp(vals, tenv), valExp.vals),
            (texp : TExp[]) => allT(isNonTupleTExp, texp)? makeOk(makeNonEmptyTupleTExp(texp)) : makeFailure("Illegal arguments"));
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const values: CExp[] = map((binding)=> binding.val, exp.bindings);
    const evaluatedValues: Result<TExp[]> = (mapResult((val) => typeofExp(val, tenv), values));
    //const valuesAsTexp: TExp[] = makeValuesAsTexp(evaluatedValues);
    const valuesAsTexp: TExp[] = isOk(evaluatedValues)?
        allT(isNonEmptyTupleTExp, evaluatedValues.value)?
            reduce(
                (acc: TExp[], currTExp: TExp) =>  isNonEmptyTupleTExp(currTExp) ? reduce(
                        (secondAcc: TExp[], valueInTExp:TExp) => secondAcc.concat(valueInTExp),
                        acc, 
                        currTExp.TEs)
                     : [currTExp], 
                [], 
                evaluatedValues.value) 
            :[] 
        :[];

    const variables: VarDecl[] = reduce((acc:VarDecl[], curr:VarDecl[])=>concat(acc,curr) , [], map((binding)=> binding.var , exp.bindings));
    
    if (valuesAsTexp.length !== variables.length)
        return makeFailure(`The number of values and the number of varibles dont match`);
    
    const TEs = map((x: VarDecl) => x.texp, variables);
    
    const constraints = zipWithResult((varTE, val) => bind(makeOk(val),(typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),TEs, valuesAsTexp);
    
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(map((x: VarDecl) => x.var, variables), TEs, tenv)));
};
