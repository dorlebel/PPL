...
import { Result, makeOk, makeFailure, bind, mapResult, safe2, safe3, safe4} from "../imp/result";
...
const parseForExp = ( vars: Sexp, params: Sexp[]): Result<ForExp> =>
    isIdentifier(vars) && params.length === 3 && (params[0] < params[1] || params[0] === params[1])?
    bind(mapResult(parseL21CExp, params),(cexps: CExp[]) => bind ( makeOk(makeVarDecl(vars)) ,
        ( loop: VarDecl) => safe3((start: NumExp, end: NumExp, body: CExp)=>
            ( makeOk(makeForExp(loop, start, end, body))))
            ( isNumExp(cexps[0]) ? makeOk(cexps[0]) : makeFailure("invalid expression"), 
            isNumExp(cexps[1]) ? makeOk(cexps[1]) : makeFailure("invalid expression"), 
            makeOk(cexps[2]))))
